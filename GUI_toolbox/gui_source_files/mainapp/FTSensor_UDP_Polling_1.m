...
% record the start time for once
if time_flag == 0
    start_time = double(FT_Sensor(1).Data.tStamp(2)) + double(FT_Sensor(1).Data.tStamp(1))/1000000;
    time_flag = 1;
end
Fx=FT_Sensor(1).Data.filt_ft(1);
Fy=FT_Sensor(1).Data.filt_ft(2);
Fz=-FT_Sensor(1).Data.filt_ft(3); 
F=[Fx;Fy;Fz];
%         display(F'); %comment this for not printing incommand window

Tx=FT_Sensor(1).Data.filt_ft(4);
Ty=FT_Sensor(1).Data.filt_ft(5);
Tz=FT_Sensor(1).Data.filt_ft(6);
T=[Tx;Ty;Tz];
%         display(T');%comment this for not printing incommand window

%------------------------Update 3D Plot ---------------------------------
FT_Sensor(1).Data.ctime = double(FT_Sensor(1).Data.tStamp(2)) + double(FT_Sensor(1).Data.tStamp(1))/1000000-start_time;
time_span = [time_span,FT_Sensor(1).Data.ctime];
delay=delay+1;
